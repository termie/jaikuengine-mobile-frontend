...
typedef struct {
    ...
    SV * handler;
    SV * saved_error;
    xmlChar char_buffer[MAX_CHAR_BUFFER];
    int char_buffer_len;
} PmmSAXVector;
...
void buffer_or_report(pTHX_ SV *handler, PmmSAXVectorPtr sax, const xmlChar * ch, int len);
...
PSaxStartElement(void *ctx, const xmlChar * name, const xmlChar** attr)
{
    ...
    SV * rv;
    SV * arv;
    buffer_or_report(aTHX_ handler, sax, 0, 0);
...
void buffer_or_report(pTHX_ SV *handler, PmmSAXVectorPtr sax, const xmlChar * ch, int len)
{
    xmlChar* to_report=0; int report_len=0;
    HV* element;
    SV * rv = NULL;

    if (ch!=NULL) {
        if ( (sax->char_buffer_len + len) > MAX_CHAR_BUFFER) {
            buffer_or_report(aTHX_ handler, sax, 0, 0);
        }
        if ( len > MAX_CHAR_BUFFER) {
            to_report=ch;
            report_len=len;
        } else {
            memcpy( sax->char_buffer+sax->char_buffer_len, ch, sizeof(xmlChar)*len);
            sax->char_buffer_len+=len;
            return;
        }
    } else {
        to_report=sax->char_buffer;
        report_len=sax->char_buffer_len;
        sax->char_buffer_len=0;
    }
    ...
    XPUSHs(handler);
    element = PmmGenCharDataSV(aTHX_ sax, to_report, report_len);
...
/*         warn( "lost my sax context!? ( %s, %d )\n", ch, len ); */
    ...
    if ( ch != NULL && handler != NULL ) {
        buffer_or_report(aTHX_ handler, sax, ch, len);
    }
