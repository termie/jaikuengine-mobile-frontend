...
my $debugout;
#$debugout=IO::Handle->new;
#open ($debugout, ">>/home/jaiku/djabberd-debugout") || die "cannot open /home/jaiku/djabberd-debugout";
...
sub borrow_a_parser {
    ...
    if ($self->{in_stream}) {
        my $ns = $self->namespace . $self->extra_namespace;
	...
	if ($status != Z_OK) { 
		...
		if (defined($debug)) {
			...
			print "undef\n";
                ...
                close(DEBUG);
                $self->stream_error("internal-server-error", "error in deflate $status");
                return 1;
	}
	$data=$r;
	if ( defined($debug) ) {
		...
		if ($status != Z_OK) { 
			...
			print DEBUG $debug, "\n";
			close(DEBUG);
			$self->stream_error("internal-server-error", "error in flush $status");
			return 1;
		}
		$data .= $r;
	}
...
sub debugout {
	return unless($debugout);
	my $self=shift;
	my $bref=shift;
        my $time = Time::HiRes::time;
	my $id=$self->{id};
	$debugout->print("$time $id $bref\n");
}
# DJabberd::Connection
sub event_read {
    my DJabberd::Connection $self = shift;
	...
	if ( substr($$bref, 0, 1) ne "<" ) {
	    ...
	    if ($status != Z_OK) { 
		die "error in deflateInit"; }
	    ($self->{istream}, $status) = inflateInit();
	    if ($status != Z_OK) { die "error in inflateInit"; }
	}
    }
    if ($self->{do_compress}) {
