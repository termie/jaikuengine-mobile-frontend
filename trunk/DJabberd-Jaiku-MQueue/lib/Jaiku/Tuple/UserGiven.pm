...
sub make {
	my ($description, $since, $timestamp)=@_;

	my $u=new Jaiku::BBData::UserGiven("usergiven");
	$u->since->set_value($since);
	$u->description->set_value($description);

	my $e=new Jaiku::BBData::SensorEvent;
	$e->name->set_value("usergiven");
	$e->stamp->set_value($timestamp);
	$e->priority->set_value(2);
	$e->set_data($u);

	my $t=new Jaiku::BBData::Tuple;
	$t->tuplemeta->moduleuid->set_value("0x20006E4D");
	$t->tuplemeta->moduleid->set_value(33);
	$t->tupleid->set_value(11);
