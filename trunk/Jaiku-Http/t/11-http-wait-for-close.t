...
# Giving a coderef as the content when the client is HTTP 1.0 makes
# HTTP::Daemon use neither Content-length nor chunked so we can test correct
# handling of just waiting for the connection to close.
my $responder = sub {
  my ($conn, $request) = @_;
  my $offset = 0;
  my $chunk_length = 10;
