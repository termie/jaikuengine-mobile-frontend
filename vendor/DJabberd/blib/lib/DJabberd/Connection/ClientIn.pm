...
#       print STDERR "ACK: " . $node->as_xml . "\n";
    ...
    if ($node->element eq "{http://www.cs.helsinki.fi/group/context}tuple") {
        ...
        if (Jaiku::Tuple::ThreadRequest::is_threadrequest($node->tuplemeta->moduleuid->value,
            ...
            $self->{messagequeue}->received_request( $self->server->{c2s_port},
                    $self->bound_jid->node, $self->{device_id},
                    $node, sub { $self->ack_to_client(@_) } );
...
sub ack_to_client {
    ...
    return unless($module_uid);

    if (Jaiku::Tuple::ThreadRequest::is_threadrequest($module_uid, $module_id)) {
        my $reply=Jaiku::Tuple::ThreadRequestReply::make($subname, $extra);
        $self->write($reply->as_xml);
    }
}
...
sub set_bound_jid {
    my ($self, $jid, $auth_params) = @_;
    my $self->{auth_params} = $auth_params;
