...
int BitmapCompiler::WriteFilebitmap(SEpocBitmapHeader* aPbm)
	...
	// Teemu 1.11.2006 
	// WriteBuf16() cannot handle odd number of bytes 
...
int BitmapCompiler::CompressTwentyFourBitData(char*& aDest,char* aSrce,int aSizeInBytes)
	...
	while (aSrce < srceLimitPtrMinusThree)
		...
		if (TrueColorPointerCompare(aSrce,component1,component2,component3))
			...
			int len = (aSrce - runStartPtr);
 			if (aDest + len > destCompressedLimitPtr)
 				return NoCompression;
			ret = WriteCompressedTwentyFourBitValues(aDest,component1,component2,component3,len / 3);
			if (ret)
				return ret;
			}
		else
			...
			int len = (aSrce - runStartPtr);
 			if (aDest + len > destCompressedLimitPtr)
 				return NoCompression;
			ret = WriteCompressedTwentyFourBitData(aDest,runStartPtr, len / 3);
			if (ret)
				return ret;
			}
		if (aDest > destCompressedLimitPtr)
