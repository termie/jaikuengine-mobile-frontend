...
sub AddTimer {
    ...
    if ($secs>=1) {
        my $fire_time = POSIX::ceil(Time::HiRes::time()+$secs);
        my $now=int(Time::HiRes::time());
        $secs=int($secs);
        unless ( $SlowTimerStart ) {
            $SlowTimerStart=$now+1;
        }
        $fire_time -= $SlowTimerStart;
        my $timer = bless [$fire_time, $coderef], "Danga::Socket::Timer";
        push (@{$SlowTimers[$fire_time]}, $timer);
        return $timer;
    }
...
my $shiftcount;
sub RunTimers {
    return $LoopTimeout unless (@Timers || @SlowTimers);
    ...
    #print STDERR "RunTimers now $now slowstart $SlowTimerStart\n";
    if ($SlowTimerStart && $SlowTimerStart <= $now) {
        my $slow=shift @SlowTimers;
        $shiftcount++;
        if ($shiftcount>1000) {
            # stop array from growing forever
            my @temp=@SlowTimers;
            @SlowTimers=@temp;
            $shiftcount=0;
        }
        foreach my $to_run (@$slow) {
            $to_run->[1]->($now) if $to_run->[1];
        }
        if (@SlowTimers) {
            $SlowTimerStart += 1;
        } else {
            $SlowTimerStart = undef;
        }
    }
    ...
    unless (@Timers) {
        return $LoopTimeout if ($LoopTimeout<1000 && $LoopTimeout!=-1);
        return 1000;
    }
