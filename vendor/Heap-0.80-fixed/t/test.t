...
BEGIN {
    ...
    print "1..", ($b1*2*8+4)*($fibi+$biny+$binl)*2+1, "\n";
}
END {print "not ok 1\n" unless $loaded;}
use Heap;
$loaded = 1;
...
my $num_elementf = sub { return NumElem(@_); };

$fibi && testaheap( Heap::Fibonacci->new, $num_elementf );
$binl && testaheap( Heap::Binomial->new, $num_elementf );
$biny && testaheap( Heap::Binary->new, $num_elementf );

my $str_elementf = sub { return StrElem(sprintf("%06d", $_[0])); };
$fibi && testaheap( Heap::Fibonacci->new, $str_elementf );
$binl && testaheap( Heap::Binomial->new, $str_elementf );
$biny && testaheap( Heap::Binary->new, $str_elementf );
